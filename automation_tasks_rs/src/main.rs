//! automation_tasks_rs for bestia_dev_new_rust_wasm_pwa
...
fn task_build() {
    ...
    run_shell_command("\\rsync -a --delete-after pkg/ web_server_folder/bestia_dev_new_rust_wasm_pwa/pkg/");
    println!(
        r#"{YELLOW}
...
fn task_publish_to_web() {
    ...
    let shell_command = format!(
        "rsync -e ssh -a --info=progress2 --delete-after ~/rustprojects/{package_name}/web_server_folder/ luciano_bestia@bestia.dev:/var/www/bestia.dev/{package_name}/",
        package_name = cargo_toml.package_name()
    );
    run_shell_command(&shell_command);
    println!(
